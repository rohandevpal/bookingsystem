...
//   await prisma.review.deleteMany();
  ...
  await prisma.restorent.deleteMany();
...
//   await prisma.user.deleteMany();
  ...
  await prisma.restorent.createMany({
    data: [
      // INDIAN //
      {
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "With 20 years of experience cooking in the finest restorents, our chef is excited to present their vision to you and all our guests. Our caring and committed staff will ensure you have a fantastic experience with us.",
        images: [
        ...
        slug: "ramakrishna-indian-restorent-ottawa",
        location_id: ottawaLocationId,
        region_id: indianRegionId,
      },
      {
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Welcome to Last Train to Delhi. We are a progressive Indian restorent in the beautiful Glebe community in Ottawa. Our speciality is Northern Indian food, classics like Murg Mahkini and some modern dishes like Crispy Shrimp. We are a small cozy restorent, so make sure that you reserve through OpenTable.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "At this refined, yet casual, Indian restorent, the portions are large, the wine list is top-notch, and the ambience encourages sharing.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "This elegant fine dining Indian restorent has been satisfying the Indian tandoori and curry cravings for 12 years in Toronto.",
        images: [
        ...
        open_time: "10:00:00.000Z",
        close_time: "21:00:00.000Z",
        slug: "kamasutra-indian-restorent-and-wine-bar-niagara",
        location_id: niagaraLocationId,
        region_id: indianRegionId,
      },
      // MEXICAN //
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Eldorado Taco restorent is excited to serve you traditional Mexican region, re-imagined with a distinct modern flair, in a stylish setting on Preston street. Striving to bring you some of Ottawa’s best Tacos, margaritas and Tequila. Reserve your table now!",
        images: [
      ...
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "The first location in Canada, from famed restorenteurs Noble 33, welcomes patrons into an immersive dining experience.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Stelvio on Dundas West is an authentic Italian restorent serving classic old world fare using traditional recipes and ingredients. Recipes have been fine-tuned to satisfy the palate of the modern guest, and fresh meals are prepared daily.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "ēst is a modern, newly reopened restorent serving Italian-French courses, captivating cocktails and wine.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "Welcome to the newest edition to College street iL PADRINO Ristorante has joined the list of Italian restorents where Chef Connie award winning Italian Chef makes every Italian dish with love like no other. ",
        images: [
  ...
  const restorents = await prisma.restorent.findMany();

  const vivaanId =
    restorents.find((restorent) => restorent.name === "Vivaan - fine Indian")
      ?.id || 1;
  const RamaKrishnaId =
    restorents.find((restorent) => restorent.name === "RamaKrishna Indian")
      ?.id || 1;
  const coconutLagoonId =
    restorents.find((restorent) => restorent.name === "Coconut Lagoon")
      ?.id || 1;
  const lastTrainToDelhiId =
    restorents.find((restorent) => restorent.name === "Last Train to Delhi")
      ?.id || 1;
  const adrakYorkvilleId =
    restorents.find((restorent) => restorent.name === "Adrak Yorkville")
      ?.id || 1;
  const curryishTavernId =
    restorents.find((restorent) => restorent.name === "Curryish Tavern")
      ?.id || 1;
  const utsavId =
    restorents.find((restorent) => restorent.name === "Utsav")?.id || 1;
  const pukkaId =
    restorents.find((restorent) => restorent.name === "Pukka")?.id || 1;
  const kamasutraIndianId =
    restorents.find((restorent) => restorent.name === "Kamasutra Indian")
      ?.id || 1;
  const eldoradoTacoId =
    restorents.find((restorent) => restorent.name === "Eldorado Taco")?.id ||
    1;
  const laBartolaId =
    restorents.find((restorent) => restorent.name === "La Bartola")?.id || 1;
  const elCatrinId =
    restorents.find((restorent) => restorent.name === "El Catrin")?.id || 1;
  const mariachisId =
    restorents.find((restorent) => restorent.name === "3 Mariachis")?.id ||
    1;
  const canorestorentId =
    restorents.find((restorent) => restorent.name === "Cano restorent")
      ?.id || 1;
  const bluRistoranteId =
    restorents.find((restorent) => restorent.name === "Blu Ristorante")
      ?.id || 1;
  const stelvioId =
    restorents.find((restorent) => restorent.name === "Stelvio")?.id || 1;
  const sofiaId =
    restorents.find((restorent) => restorent.name === "Sofia")?.id || 1;
  ...
  await prisma.item.createMany({
    data: [
      ...
      },
      {
        ...
        price: "$12.00",
        restorent_id: lastTrainToDelhiId,
      },
      {
      ...
      },
      {
        ...
        price: "$8.99",
        restorent_id: kamasutraIndianId,
      },
      {
